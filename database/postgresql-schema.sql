...
-- QUERY START:
create or replace function TigSocks5CreateUid(varchar(2049))
	returns bigint as '
declare
	_user_id alias for $1;
	_res_uid bigint;
begin	
	insert into tig_socks5_users (user_id) 
		values (_user_id);
...
-- QUERY START:
create or replace function TigSocks5TransferUsedGeneral(varchar(2049))
	returns bigint as '
declare
	_user_id alias for $1;
	_res bigint;
...
begin	
	select sum(transferred_bytes) into _res from tig_socks5_connections
		where transfer_timestamp > to_char(now(), ''YYYY-MM-01'')
		and uid in (select uid from tig_socks5_users where user_id like (''%'' || _domain));
...
-- QUERY START:
create or replace function TigSocks5CreateTransferUsed(bigint, int)
	returns bigint as '
declare
	_uid alias for $1;
